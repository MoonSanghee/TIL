...
points = []
points.append((xs, ys))
time = [[float('inf') for _ in range(8)] for _ in range(8)]
...
for _ in range(3):
    ...
    points.append((x1, y1))
    points.append((x2, y2))
    time[len(points) - 1][len(points) - 2] = time[len(points) - 2][len(points) - 1] = min(simulate((x1, y1), (x2, y2)), 10)
    # 텔레포트 좌표를 받고 텔레포트 하는 시간과 점프해서 이동하는 경우를 비교해서 시간이 짧은쪽을 담아줍니다.

points.append((xe, ye))
# 도착 좌표를 담아줍니다.
time[0][7] = time[7][0] = simulate(points[0], points[7])
